    ...
    for(size_t t_i=0; t_i < N; ++t_i) {
        ...
        for(size_t x_i = 1; x_i < side_length-1; ++x_i) {
            for(size_t y_i = 1; y_i < side_length-1; ++y_i) {
                const BUF_TYPE up = t1[Idx(x_i, y_i-1, side_length)];
                const BUF_TYPE down = t1[Idx(x_i, y_i+1, side_length)];
                const BUF_TYPE left = t1[Idx(x_i-1, y_i, side_length)];
                const BUF_TYPE right = t1[Idx(x_i+1, y_i, side_length)];
                const BUF_TYPE center = t1[Idx(x_i, y_i, side_length)];
                const BUF_TYPE lap = (up+down+left+right-4*center)/(dx*dx);
                const BUF_TYPE tdiff = alpha*lap;
                // Compute update
                t2[Idx(x_i, y_i, side_length)] = t1[Idx(x_i, y_i, side_length)]+tdiff;
            }
        }
